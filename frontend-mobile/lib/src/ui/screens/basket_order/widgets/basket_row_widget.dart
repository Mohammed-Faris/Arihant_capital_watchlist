...
class BasketRowWidget extends BaseScreen {
  final Orders orders;
  final Function onRowClick;
  final bool isBottomSheet;
  final bool showOrdertype;
  final bool iseditBasket;
  final bool isaddBasket;
  final bool isExecuteallorder;
  final Function()? onTap;

  const BasketRowWidget(
      {Key? key,
      ...
      this.iseditBasket = false,
      this.isaddBasket = false,
      this.isExecuteallorder = true})
      : super(key: key);

  @override
  State<BasketRowWidget> createState() => _BasketRowWidgetState();
}

class _BasketRowWidgetState extends BaseAuthScreenState<BasketRowWidget> {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: EdgeInsets.only(
            bottom: widget.iseditBasket ? 0 : 7.w,
            top: widget.iseditBasket ? 0 : 5.w),
        decoration: BoxDecoration(
          border: Border(
            bottom: widget.iseditBasket
                ? BorderSide.none
                : BorderSide(
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          width: (orders.sym?.dispSym ?? orders.dispSym)?.textSize(
            (orders.sym?.optionType != null)
                ? '${orders.sym?.baseSym ?? orders.baseSym} '
                : AppUtils()
                    .dataNullCheck((orders.sym?.dispSym ?? orders.dispSym)),
            Theme.of(context)
                .primaryTextTheme
                .labelSmall!
        ...
        FandOTag(orders),
        if (orders.ordDuration?.toLowerCase() ==
    ...
    return Container(
      ...
      child: FittedBox(
        ...
        child: Row(
          children: [
            statusField2(
                orders.prdType == "NORMAL"
                    ? AppConstants.carryForward
                    : orders.prdType.toString(),
                AppConstants.none,
                false),
  ...
  Widget _buildOrdActionAndQtyWidget(Orders orders) {
    return Padding(
      ...
      child: (!(widget.isExecuteallorder) &&
              ((orders.isModifiable ?? false) ||
                  !(orders.isExecutable ?? false)) &&
              widget.isaddBasket)
          ? GestureDetector(
              onTap: widget.onTap ?? () {},
              child: Container(
  ...
  Widget _buildBottomRowWidget(Orders orders) {
    return Row(
      ...
      children: [
        ...
        if (widget.isBottomSheet) _buildOrdActionAndQtyWidget(orders),
        if (widget.isaddBasket || widget.iseditBasket) _buildLtpWidget(orders),
      ],
    );
  }
